...
_Preference_: Only _non-compliant_ observations as those are the ones that are of real interest and need proper follow-up action. And it drastically reduces the amount of observations to be processed and stored.
...
_Preference_: Do not include actual data in the observation results, but allow for a reference to where the actual data can be found (e.g., a query, a file, etc.). This way, the observation remains lightweight and secure, while still providing a way to access the relevant data when needed.
...
_Preference_: Make OORS focused on data quality observations only (and possibly rename it accordingly).
